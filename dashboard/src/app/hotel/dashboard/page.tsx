    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
          Badger Hotel — Dashboard
        </h1>
        <p className="text-sm text-gray-500 mt-1">
          Badger Hotel Group operating performance
        </p>
      </div>

      <div className="flex flex-wrap items-center justify-between gap-3">
        <ExportButtons
          fileName="Badger_Hotel_Dashboard"
          title="Badger Hotel Dashboard"
          headers={["Section", "Account", "Amount"]}
          rows={[
            ...revenueAccounts.map((a) => ["Revenue", a.name, fmt(a.ytd)]),
            ...expenseAccounts.map((a) => ["Expense", a.name, fmt(Math.abs(a.ytd))]),
          ]}
        />
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <KpiCard label="Room Revenue" value={fmtK(totalRevenue)} color="text-green-600" />
            <KpiCard label="Operating Expenses" value={fmtK(totalExpenses)} color="text-red-600" />
            <KpiCard
              label="Net Income"
                  ...
                  >
                    {columns.map((col) =>
                      col.key === "account" ? (
                        ...
                        <td key={col.key} className={`px-4 py-2 text-right font-mono ${colorClass}`}>
                          {fmt(displayAmount)}
                        </td>
                      )
                    )}
