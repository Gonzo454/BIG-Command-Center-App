...
function analyzeTransactions(txns) {
  ...
  const topGLAccounts = Object.entries(byGL).sort((a, b) => b[1] - a[1]).slice(0, 10);
  const glDetails = topGLAccounts.map(([gl, total]) => {
    const txnList = (glTransactions[gl] || [])
      .sort((a, b) => b.amt - a.amt);
    return { gl, total, transactions: txnList };
  });

  return { totalDisbursed, topProperties, topVendors, topVendorDetails, topGLAccounts, glDetails, flags };
}

function buildEmail(txns, reportDate) {
  const { totalDisbursed, topProperties, topVendors, topVendorDetails, topGLAccounts, glDetails, flags } = analyzeTransactions(txns);
  ...
  const glDetailRows = glDetails.map(g => {
    const txnRows = g.transactions.map(t => `
      <tr>
        <td style="padding:4px 12px 4px 16px;font-size:12px;color:#374151;border-bottom:1px solid #f3f4f6;">${t.vendor}</td>
        <td style="padding:4px 8px;font-size:12px;color:#6B7280;border-bottom:1px solid #f3f4f6;">${t.prop}</td>
        <td style="padding:4px 8px;font-size:12px;color:#6B7280;border-bottom:1px solid #f3f4f6;">${t.date}</td>
        <td style="padding:4px 0;font-size:12px;color:#111827;text-align:right;border-bottom:1px solid #f3f4f6;">${fmtFull(t.amt)}</td>
      </tr>`).join('');
    return `
  <tr><td style="padding:20px 28px 0;">
    <h3 style="margin:0 0 8px;font-size:13px;font-weight:600;color:#111827;">${g.gl} <span style="font-weight:400;color:#6B7280;">— ${fmtFull(g.total)} total (${g.transactions.length} transactions)</span></h3>
    <table width="100%" cellpadding="0" cellspacing="0">
      <tr>
        <td style="padding:4px 12px 4px 16px;font-size:11px;color:#9CA3AF;text-transform:uppercase;letter-spacing:0.04em;border-bottom:1px solid #E5E7EB;">Vendor</td>
        <td style="padding:4px 8px;font-size:11px;color:#9CA3AF;text-transform:uppercase;letter-spacing:0.04em;border-bottom:1px solid #E5E7EB;">Property</td>
        <td style="padding:4px 8px;font-size:11px;color:#9CA3AF;text-transform:uppercase;letter-spacing:0.04em;border-bottom:1px solid #E5E7EB;">Date</td>
        <td style="padding:4px 0;font-size:11px;color:#9CA3AF;text-transform:uppercase;letter-spacing:0.04em;text-align:right;border-bottom:1px solid #E5E7EB;">Amount</td>
      </tr>
      ${txnRows}
    </table>
...
<table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:8px;border:1px solid #E5E7EB;">
  ...
  <tr><td style="padding:28px 28px 0;">
    <h2 style="margin:0 0 4px;font-size:13px;font-weight:500;color:#6B7280;text-transform:uppercase;letter-spacing:0.06em;">Top 10 GL accounts — transaction detail</h2>
    <p style="margin:0 0 12px;font-size:12px;color:#9CA3AF;">Every transaction in each major expense category</p>
  </td></tr>
  ${glDetailRows}
